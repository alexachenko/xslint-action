...
test:
	...
	echo "$$output" | grep -q "Processed files: 2" || exit 1; echo "$$output" | grep -q "Defects found: 88" || exit 1; echo "$$output" | grep -q "Directories and files to process: xsl-packs/xsl-with-no-violations.xsl, xsl-packs/xsl-with-some-violations.xsl" || exit 1;\
